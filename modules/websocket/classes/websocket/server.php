...
class Websocket_Server {
    ...
    /*
      $this->wsClients[ integer socket_id ] = array(
      ...
      $wsRead[ integer socket_id ] = resource Socket         // this one-dimensional array is used for socket_select()
    ...
    function wsStartServer($host, $port) {
        ...
        if (!$this->wsRead[0] = socket_create(AF_INET, SOCK_STREAM, SOL_TCP)) {
            $this->log('Nie udało się utworzyć gniazda');
            return false;
        }

        if (!socket_set_nonblock($this->wsRead[0])) {
            $this->log('Nie udało się ustawić gniazda w tryb nieblokujący');
            return false;
        }

        if (!socket_set_option($this->wsRead[0], SOL_SOCKET, SO_REUSEADDR, 1)) {
            $this->log('Nie udało się ustawić gniazda w tryb TCP/IP z recyklicznym użyciem adresów');
            socket_close($this->wsRead[0]);
            return false;
        }
        if (!socket_bind($this->wsRead[0], $host, $port)) {
            $this->log('Nie udało się połączyć gniazda z ip i portem');
            socket_close($this->wsRead[0]);
            return false;
        }
        if (!socket_listen($this->wsRead[0], 10)) {
            $this->log('Nie udało się przełączyć gniazda w tryb nasłuchu');
            socket_close($this->wsRead[0]);
            return false;
        }
        ...
        while (isset($this->wsRead[0])) {
            ...
            if ($result === false) {
                ...
                foreach ($changed as $socket_id => $socket) {
                    if ($socket_id != 0) {
                        ...
                        if ($bytes === false) {
                            ...
                            $this->wsSendClientClose($socket_id, self::WS_STATUS_PROTOCOL_ERROR);
                            $this->log('error on recv, remove client socket (will check to send close frame)');
                        } elseif ($bytes > 0) {
                            // process handshake or frame(s)
                            if (!$this->wsProcessClient($socket_id, $buffer, $bytes)) {
                                $this->wsSendClientClose($socket_id, self::WS_STATUS_PROTOCOL_ERROR);
                            }
                        } else {
                            // 0 bytes received from client, meaning the client closed the TCP connection
                            $this->wsRemoveClient($socket_id);
                        }
                    } else {
    ...
    function wsStopServer() {
        ...
        foreach ($this->wsClients as $socket_id => $client) {
            ...
            if ($client[2] != self::WS_READY_STATE_CONNECTING) {
                $this->wsSendClientClose($socket_id, self::WS_STATUS_GONE_AWAY);
            }
            ...
            if ($client[2] != self::WS_READY_STATE_CLOSED) {
                ...
                if ($client[4] !== false) {
                    ...
                    if ($time >= $client[4] + self::WS_TIMEOUT_PONG) {
                        ...
                        $this->wsSendClientClose($socket_id, self::WS_STATUS_TIMEOUT);
                        $this->wsRemoveClient($socket_id);
                    }
                    ...
                    if ($client[2] != self::WS_READY_STATE_CONNECTING) {
                        ...
                        $this->wsClients[$socket_id][4] = time();
                        $this->wsSendClientMessage($socket_id, self::WS_OPCODE_PING, '');
                    } else {
    ...
    function wsAddClient($socket, $clientIP) {
        ...
        $socket_id = $this->wsGetNextsocket_id();

        // store initial client data
        $this->wsClients[$socket_id] = array($socket, '', self::WS_READY_STATE_CONNECTING, time(), false, 0, $clientIP, false, 0, '', 0, 0);

        // store socket - used for socket_select()
        $this->wsRead[$socket_id] = $socket;
    }

    function wsRemoveClient($socket_id) {
        // fetch close status (which could be false), and call wsOnClose
        $closeStatus = $this->wsClients[$socket_id][5];
        if (array_key_exists('close', $this->wsOnEvents))
            foreach ($this->wsOnEvents['close'] as $func)
                $func($socket_id, $closeStatus);

        // close socket
        $socket = $this->wsClients[$socket_id][0];
        socket_close($socket);

        // decrease amount of clients connected on this client's IP
        $clientIP = $this->wsClients[$socket_id][6];
        ...
        unset($this->wsRead[$socket_id], $this->wsClients[$socket_id]);
    }

    // client data functions
    function wsGetNextsocket_id() {
    ...
    function wsGetClientSocket($socket_id) {
        return $this->wsClients[$socket_id][0];
    }

    // client read functions
    function wsProcessClient($socket_id, &$buffer, $bufferLength) {
        if ($this->wsClients[$socket_id][2] == self::WS_READY_STATE_OPEN) {
            // handshake completed
            $result = $this->wsBuildClientFrame($socket_id, $buffer, $bufferLength);
        } elseif ($this->wsClients[$socket_id][2] == self::WS_READY_STATE_CONNECTING) {
            // handshake not completed
            $result = $this->wsProcessClientHandshake($socket_id, $buffer);
            if ($result) {
                $this->wsClients[$socket_id][2] = self::WS_READY_STATE_OPEN;
    ...
    function wsBuildClientFrame($socket_id, &$buffer, $bufferLength) {
        // increase number of bytes read for the frame, and join buffer onto end of the frame buffer
        $this->wsClients[$socket_id][8] += $bufferLength;
        $this->wsClients[$socket_id][9] .= $buffer;

        // check if the length of the frame's payload data has been fetched, if not then attempt to fetch it from the frame buffer
        if ($this->wsClients[$socket_id][7] !== false || $this->wsCheckSizeClientFrame($socket_id) == true) {
            // work out the header length of the frame
            $headerLength = ($this->wsClients[$socket_id][7] <= 125 ? 0 : ($this->wsClients[$socket_id][7] <= 65535 ? 2 : 8)) + 6;

            // check if all bytes have been received for the frame
            $frameLength = $this->wsClients[$socket_id][7] + $headerLength;
            if ($this->wsClients[$socket_id][8] >= $frameLength) {
                // check if too many bytes have been read for the frame (they are part of the next frame)
                $nextFrameBytesLength = $this->wsClients[$socket_id][8] - $frameLength;
                if ($nextFrameBytesLength > 0) {
                    $this->wsClients[$socket_id][8] -= $nextFrameBytesLength;
                    $nextFrameBytes = substr($this->wsClients[$socket_id][9], $frameLength);
                    $this->wsClients[$socket_id][9] = substr($this->wsClients[$socket_id][9], 0, $frameLength);
                }

                // process the frame
                $result = $this->wsProcessClientFrame($socket_id);

                // check if the client wasn't removed, then reset frame data
                if (isset($this->wsClients[$socket_id])) {
                    $this->wsClients[$socket_id][7] = false;
                    $this->wsClients[$socket_id][8] = 0;
                    $this->wsClients[$socket_id][9] = '';
                }
                ...
                return $this->wsBuildClientFrame($socket_id, $nextFrameBytes, $nextFrameBytesLength);
            }
        }
    ...
    function wsCheckSizeClientFrame($socket_id) {
        // check if at least 2 bytes have been stored in the frame buffer
        if ($this->wsClients[$socket_id][8] > 1) {
            // fetch payload length in byte 2, max will be 127
            $payloadLength = ord(substr($this->wsClients[$socket_id][9], 1, 1)) & 127;

            if ($payloadLength <= 125) {
                // actual payload length is <= 125
                $this->wsClients[$socket_id][7] = $payloadLength;
            } elseif ($payloadLength == 126) {
                // actual payload length is <= 65,535
                if (substr($this->wsClients[$socket_id][9], 3, 1) !== false) {
                    // at least another 2 bytes are set
                    $payloadLengthExtended = substr($this->wsClients[$socket_id][9], 2, 2);
                    $array = unpack('na', $payloadLengthExtended);
                    $this->wsClients[$socket_id][7] = $array['a'];
                }
            } else {
                // actual payload length is > 65,535
                if (substr($this->wsClients[$socket_id][9], 9, 1) !== false) {
                    // at least another 8 bytes are set
                    $payloadLengthExtended = substr($this->wsClients[$socket_id][9], 2, 8);
                    ...
                    if ($array['a'] != 0 || ord(substr($payloadLengthExtended, 4, 1)) & 128) {
                        $this->wsSendClientClose($socket_id, self::WS_STATUS_MESSAGE_TOO_BIG);
                        return false;
                    }
            ...
            if ($this->wsClients[$socket_id][7] !== false) {

                // check if the frame's payload data length exceeds self::WS_MAX_FRAME_PAYLOAD_RECV
                if ($this->wsClients[$socket_id][7] > self::WS_MAX_FRAME_PAYLOAD_RECV) {
                    $this->wsClients[$socket_id][7] = false;
                    $this->wsSendClientClose($socket_id, self::WS_STATUS_MESSAGE_TOO_BIG);
                    return false;
                }
                ...
                $controlFrame = (ord(substr($this->wsClients[$socket_id][9], 0, 1)) & 8) == 8;
                if (!$controlFrame) {
                    $newMessagePayloadLength = $this->wsClients[$socket_id][11] + $this->wsClients[$socket_id][7];
    ...
    function wsProcessClientFrame($socket_id) {
        // store the time that data was last received from the client
        $this->wsClients[$socket_id][3] = time();

        // fetch frame buffer
        $buffer = &$this->wsClients[$socket_id][9];
...
// fetch byte position where the mask key starts
        $seek = $this->wsClients[$socket_id][7] <= 125 ? 2 : ($this->wsClients[$socket_id][7] <= 65535 ? 4 : 10);
        ...
        if ($opcode != self::WS_OPCODE_CONTINUATION && $this->wsClients[$socket_id][11] > 0) {
            // clear the message buffer
            $this->wsClients[$socket_id][11] = 0;
            $this->wsClients[$socket_id][1] = '';
        }
        ...
        if ($fin == self::WS_FIN) {
            ...
            if ($opcode != self::WS_OPCODE_CONTINUATION) {
                // process the message
                return $this->wsProcessClientMessage($socket_id, $opcode, $data, $this->wsClients[$socket_id][7]);
            } else {
                // increase message payload data length
                $this->wsClients[$socket_id][11] += $this->wsClients[$socket_id][7];

                // push frame payload data onto message buffer
                $this->wsClients[$socket_id][1] .= $data;

                // process the message
                $result = $this->wsProcessClientMessage($socket_id, $this->wsClients[$socket_id][10], $this->wsClients[$socket_id][1], $this->wsClients[$socket_id][11]);

                // check if the client wasn't removed, then reset message buffer and message opcode
                if (isset($this->wsClients[$socket_id])) {
                    $this->wsClients[$socket_id][1] = '';
                    $this->wsClients[$socket_id][10] = 0;
                    $this->wsClients[$socket_id][11] = 0;
                }
            ...
            if ($opcode != self::WS_OPCODE_CONTINUATION) {
                $this->wsClients[$socket_id][10] = $opcode;
            }
        }
    ...
    function wsProcessClientMessage($socket_id, $opcode, &$data, $dataLength) {
        // check opcodes
        if ($opcode == self::WS_OPCODE_PING) {
            // received ping message
            return $this->wsSendClientMessage($socket_id, self::WS_OPCODE_PONG, $data);
            ...
            if ($this->wsClients[$socket_id][4] !== false) {
                $this->wsClients[$socket_id][4] = false;
            }
            ...
            if ($this->wsClients[$socket_id][2] == self::WS_READY_STATE_CLOSING) {
                ...
                $this->wsClients[$socket_id][2] = self::WS_READY_STATE_CLOSED;
            } else {
                // the server has not already sent a close frame to the client, send one now
                $this->wsSendClientClose($socket_id, self::WS_STATUS_NORMAL_CLOSE);
            }
            ...
            if (array_key_exists('message', $this->wsOnEvents))
                foreach ($this->wsOnEvents['message'] as $func)
                    $func($socket_id, $data, $dataLength, $opcode == self::WS_OPCODE_BINARY);
        }
        else {
            // unknown opcode
            return false;
    ...
    /**
     * Extract GET parameters from handshake header.
     * @param string $buffer
     * @return array
     */
    protected function httpGetArray($buffer) {
    ...
    protected function wsProcessClientHandshake($socket_id, &$buffer) {
...
// work out hash to use in Sec-WebSocket-Accept reply header
        ...
        /** Run AUTH functions. If ANY of AUTH functions return FALSE,
         * user connection will be broken.
         */
        if (array_key_exists('auth', $this->wsOnEvents)) {
            $_GET_DATA = $this->httpGetArray($buffer);
            foreach ($this->wsOnEvents['auth'] as $func) {
                if ($func($socket_id, $_GET_DATA) === false) {
                    $this->log("[socket: {$socket_id}] DISCONNECTING (AUTHORIZATION FAILED)");
                    return false;
                }
            }
        }
    ...
    protected function wsSendClientMessage($socket_id, $opcode, $message) {
        // check if client ready state is already closing or closed
        if ($this->wsClients[$socket_id][2] == self::WS_READY_STATE_CLOSING || $this->wsClients[$socket_id][2] == self::WS_READY_STATE_CLOSED)
            return true;
    ...
    protected function wsSendClientClose($socket_id, $status = false) {
        // check if client ready state is already closing or closed
        if ($this->wsClients[$socket_id][2] == self::WS_READY_STATE_CLOSING || $this->wsClients[$socket_id][2] == self::WS_READY_STATE_CLOSED)
            return true;

        // store close status
        $this->wsClients[$socket_id][5] = $status;

        // send close frame to client
        $status = $status !== false ? pack('n', $status) : '';
        $this->wsSendClientMessage($socket_id, self::WS_OPCODE_CLOSE, $status);

        // set client ready state to closing
        $this->wsClients[$socket_id][2] = self::WS_READY_STATE_CLOSING;
    }

    // client non-internal functions
    function wsClose($socket_id) {
        return $this->wsSendClientClose($socket_id, self::WS_STATUS_NORMAL_CLOSE);
    }

    function wsSend($socket_id, $message, $binary = false) {
        return $this->wsSendClientMessage($socket_id, $binary ? self::WS_OPCODE_BINARY : self::WS_OPCODE_TEXT, $message);
    }
    ...
    public function onOpen($func) {
        return $this->bind('open', $func);
    }

    public function onClose($func) {
        return $this->bind('close', $func);
    }

    public function onAuth($func) {
        return $this->bind('auth', $func);
    }

    public function onMessage($func) {
        return $this->bind('message', $func);
    }

    protected function bind($type, $func) {
        if (!isset($this->wsOnEvents[$type]))
            $this->wsOnEvents[$type] = array();
        $this->wsOnEvents[$type][] = $func;
    }

    protected function unbind($type = '') {
        if ($type)
